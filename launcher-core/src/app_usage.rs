...
impl AppUsage {
    /// `get_usage` 获取 usage 数据
    pub fn get_usage(env: &Env) -> Self {
    ...
    fn read_usage_file(env: &Env) -> Result<Self> {
        let usage_path = env.get_usage_file_path();
    ...
    fn save(&self, env: &Env) -> Result<()> {
        ...
        if let Err(err) = std::fs::write(&usage_path, &json_str) {
            if err.kind() == std::io::ErrorKind::NotFound {
                let usage_dir = env.get_usage_dir();
